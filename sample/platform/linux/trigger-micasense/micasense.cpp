...
class ImageCapture {
public:
    ImageCapture(const std::string& ip, const std::string& binary)
        : micasense_ip(ip), band_binary(binary) {}
...
private:
    std::string micasense_ip;
    std::string band_binary;
...
int main() {
    ...
    std::string band_binary = "7"; // 31 is the binary representation of 5 bands (11111)

    ImageCapture capture(micasense_ip, band_binary);
    capture.captureAndSaveImages();
